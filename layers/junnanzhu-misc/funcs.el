...
(defun junnanzhu/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun junnanzhu/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun junnanzhu/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun junnanzhu/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun junnanzhu/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "junnan")))

(defun junnanzhu/save-my-layout ()
  (interactive)
  (persp-save-state-to-file (concat persp-save-dir "junnan")))
...
(defun junnanzhu/indent-region(numSpaces)
  (progn
...
(defun junnanzhu/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (junnanzhu/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun junnanzhu/untab-region (N)
  (interactive "p")
  (junnanzhu/indent-region -4))

(defun junnanzhu/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'junnanzhu/tab-region)
  (local-set-key (kbd "<S-tab>") 'junnanzhu/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'junnanzhu/hack-tab-key)
...
(defun junnanzhu/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(junnanzhu//hotspots-sources))))

(defun junnanzhu//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (junnanzhu/run-current-file)))
...
(defun junnanzhu/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun junnanzhu/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (or (junnanzhu/vcs-project-root)
...
(defun junnanzhu/growl-notification (title message &optional sticky)
...
(defun junnanzhu/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (junnanzhu/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun junnanzhu/goto-match-paren (arg)
...
(defun junnanzhu/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun junnanzhu/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun junnanzhu/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (junnanzhu/retrieve-chrome-current-tab-url)))

(defun junnanzhu/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun junnanzhu/single-lines-only ()
...
(defun junnanzhu/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun junnanzhu/ash-term-hooks ()
...
(defalias 'tt 'junnanzhu/terminal)
...
(defun junnanzhu/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun junnanzhu/evil-quick-replace (beg end )
  (interactive "r")
...
(defun junnanzhu/vcs-project-root ()
...
(defun junnanzhu/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (junnanzhu/vcs-project-root)
                default-directory))
...
(defun junnanzhu/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (junnanzhu/vcs-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun junnanzhu/magit-visit-pull-request ()
...
(defun junnanzhu/markdown-to-html ()
  (interactive)
  (start-process "grip" "*gfm-to-html*" "grip" (buffer-file-name) "5000")
  (browse-url (format "http://localhost:5000/%s.%s" (file-name-base) (file-name-extension (buffer-file-name)))))

(defun junnan/github-browse-commit ()
...
(defun junnanzhu/search-in-fireball ()
  (interactive)
  (helm-do-ag (expand-file-name "~/Github/fireball/")))


(defun junnanzhu/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun junnanzhu/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))
