...
(defun junnanzhu/comment-box (b e)
...
(defun junnanzhu/js2-which-function ()
...
(defun junnanzhu/run-current-file ()
  ...
  (let* (
    ...
    (if (string-equal ξfSuffix "el") ; special case for emacs lisp
      ...
      (if ξprog-name
          (progn
            (message "Running…")
            (async-shell-command ξcmd-str "*junnanzhu/run-current-file output*"))
...
(defun junnanzhu/load-yasnippet ()
  (interactive)
...
(defun junnanzhu/impatient-mode-hook ()
